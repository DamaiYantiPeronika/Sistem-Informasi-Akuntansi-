<div class="content-wrapper">
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-body">
                    <form method="get" action="<?= base_url('jurnalumum') ?>" class="mb-3">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($grouped as $key => $jurnalumum): ?>
                                        <?php
                                        [$tanggalGroup, $no_trskGroup, $keteranganGroup] = explode('|', $key);
                                        // Urutkan agar sesuai urutan input (jika perlu)
                                        usort($jurnalumum, function ($a, $b) {
                                            return $a->id > $b->id ? 1 : -1;
                                        });
                                        $rowspan = count($jurnalumum);
                                        ?>
                                        <!-- Baris Header Grup -->
                                        ...
                                        <?php foreach ($jurnalumum as $ju): ?>
                                            <tr>
                                                ...
                                                <td class="text-left"><?= $ju->nama_akun ?></td>
                                                <td class="text-center"><?= $ju->no_akun ?></td>
                                                <td class="text-right">
                                                    <?= $ju->jenis_saldo == 'debit' ? 'Rp ' . number_format($ju->jumlah, 0, ',', '.') : '-' ?>
                                                </td>
                                                <td class="text-right">
                                                    <?= $ju->jenis_saldo == 'kredit' ? 'Rp ' . number_format($ju->jumlah, 0, ',', '.') : '-' ?>
                                                </td>
                                            </tr>
                                            <?php $firstRow = false; ?>
                                        <?php endforeach; ?>
