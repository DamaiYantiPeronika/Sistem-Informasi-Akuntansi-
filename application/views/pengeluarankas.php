<div class="content-wrapper">
    <section class="content">
      ...
      <h2 class="pt-3 pb-2 text-white"><?= $title; ?></h2>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="get" action="<?= base_url('pengeluarankas') ?>" class="mb-3">
                    ...
                    <!-- Tampilkan tabel pengeluaran kas -->
                    <div class="card">
                        <div class="card-body">
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    $total_kredit = 0;
                                    ?>
                                    <?php foreach ($grouped_transaksi as $groupKey => $transaksis): ?>
                                        <?php
                                        [$tanggalGroup, $no_trskGroup, $keteranganGroup] = explode('|', $groupKey);
                                        ...
                                        <?php $firstRow = true; ?>
                                        <?php foreach ($transaksis as $mk): ?>
                                            <tr>
                                                <td class="text-center"><?= $firstRow ? date('d-m-Y', strtotime($tanggalGroup)) : '' ?></td>
                                                <td class="text-center"><?= $firstRow ? $no_trskGroup : '' ?></td>
                                                <td class="text-left"><?= $mk->nama_akun ?></td>
                                                <td class="text-center"><?= $mk->no_akun ?></td>
                                                <td class="text-right">
                                                    <?= ($mk->jenis_saldo == 'debit') ? 'Rp ' . number_format($mk->jumlah, 0, ',', '.') : '-' ?>
                                                </td>
                                                <td class="text-right">
                                                    <?= ($mk->jenis_saldo == 'kredit') ? 'Rp ' . number_format($mk->jumlah, 0, ',', '.') : '-' ?>
                                                </td>
                                            </tr>
                                            <?php
                                            if ($mk->jenis_saldo == 'debit') $total_debit += $mk->jumlah;
                                            if ($mk->jenis_saldo == 'kredit') $total_kredit += $mk->jumlah;
                                            $firstRow = false;
                                            ?>
                                        <?php endforeach; ?>
                                    <?php endforeach; ?>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="4" class="text-center font-weight-bold">TOTAL PENGELUARAN KAS</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center font-weight-bold" colspan="2">
                                            ...
                                            echo ($selisih >= 0 ? 'Rp ' . number_format(abs($selisih), 0, ',', '.') : '-Rp ' . number_format(abs($selisih), 0, ',', '.'));
                                            ?>
                                        </td>
                                    </tr>
                                </tfoot>
